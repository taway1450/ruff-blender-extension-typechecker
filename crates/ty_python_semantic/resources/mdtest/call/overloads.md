...
## `overload_mapping` decorator behavior

### Direct decorator import on a function
...
```py
from decorators import overload_mapping
...
### Fully-qualified decorator on a method
...
```py
import bpy
from bpy.stub_internal.overload_mapping import overload_mapping

class Parser:
    @bpy.stub_internal.overload_mapping.overload_mapping("kind", {"int": int, "str": str})
    def parse(self, kind: str) -> object: ...

    @overload_mapping("strict", {True: int, False: str})
    def parse_with_alias(self, strict: bool) -> object: ...

parser = Parser()
reveal_type(parser.parse("int"))  # revealed: int
reveal_type(parser.parse("str"))  # revealed: str
parser.parse("other")  # error: [no-matching-overload]

reveal_type(parser.parse_with_alias(True))  # revealed: int
reveal_type(parser.parse_with_alias(False))  # revealed: str
reveal_type(parser.parse_with_alias(bool()))  # revealed: str
```
